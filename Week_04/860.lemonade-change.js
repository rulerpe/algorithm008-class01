...
var lemonadeChange = function (bills) {
  let billTalbe = [20, 10, 5];
  let registor = { 20: 0, 10: 0, 5: 0 };
  let counter = 0;
  for (let bill of bills) {
    counter++;
    let change = bill - 5;
    if (change < 0) {
      // return false if bill is less then $5
      return false;
    } else {
      // keep the bill in reigstor
      registor[bill]++;
      // skip change if bill is $5
      if (change === 0) {
        continue;
      }
    }
    // give change
    for (let b of billTalbe) {
      if (change === 0 || change < b || registor[b] === 0) {
        continue;
      } else {
        let numberOfBill = Math.floor(change / b);
        let reminder = change % b;
        let diff = registor[b] - numberOfBill;
        if (diff < 0) {
          registor[b] = 0;
          numberOfBill += diff;
        } else {
        ...
        change = b * numberOfBill + reminder;
      }
    }
    if (change !== 0) {
      return false;
